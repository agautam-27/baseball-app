...
function renderCodes(codes) {
  ...
  codes.forEach(code => {
    ...
    const isExpired = code.expirationDate && isDateExpired(code.expirationDate);
    
    // Create code item element
    const codeItem = document.createElement('div');
    codeItem.className = `code-item ${isExpired ? 'expired-item' : ''} ${code.used ? 'used-item' : ''}`;
    ...
    if (code.expirationDate) {
      codeDate.textContent = `Expiration Date: ${formatDate(code.expirationDate)}`;
      if (isExpired) {
...
async function handleAddCode() {
  try {
    ...
    const codeValue = useRandom ? generateRandomCode() : codeValueInput.value.trim();
    ...
    const codeDoc = await db.collection('coach_invites').doc(codeValue).get();
    if (codeDoc.exists) {
    ...
    await db.collection('coach_invites').doc(codeValue).set({
...
async function handleUpdateCode() {
  try {
    ...
    if (newCodeValue !== currentEditingCode.id) {
      const existingDoc = await db.collection('coach_invites').doc(newCodeValue).get();
      ...
      await db.runTransaction(async (transaction) => {
        ...
        const currentDoc = await transaction.get(db.collection('coach_invites').doc(currentEditingCode.id));
        ...
        transaction.set(db.collection('coach_invites').doc(newCodeValue), {
          ...currentData,
          expirationDate: firebase.firestore.Timestamp.fromDate(expirationDate)
        });
        
        // Delete old document
        transaction.delete(db.collection('coach_invites').doc(currentEditingCode.id));
      });
    } else {
      // Just update the expiration date
      await db.collection('coach_invites').doc(currentEditingCode.id).update({
        expirationDate: firebase.firestore.Timestamp.fromDate(expirationDate)
      });
    }
...
function confirmDeleteCode(code) {
  if (confirm(`Are you sure you want to delete the invitation code "${code.id}"?`)) {
